...
### Phase 4-A 收尾（Combat：射擊 + 子彈 + 武器素材）
...
- [ ] Cocos：Player 節點底下加結構
  - [ ] 子節點 `Weapon`（加 cc.Sprite 拉武器素材圖；不用碰撞器，純視覺）
  - [ ] `Weapon` 底下加空節點 `Muzzle`（移到槍口位置，無視覺、無碰撞器）
- [ ] Cocos：Player 節點 Add `PlayerCombat.ts`
  - [ ] Inspector：`Bullet Prefab` 拉新 Bullet.prefab
  - [ ] Inspector：`Muzzle` 拉 Player/Weapon/Muzzle 節點
  - [ ] Inspector：`Fire Cooldown` / `Pool Size` 依手感調
- [ ] Play 測試：按 J 應該射出子彈往面向方向飛、撞牆消失、跑著射方向跟著翻
- [ ] 微調 fireCooldown / bullet speed / damage
