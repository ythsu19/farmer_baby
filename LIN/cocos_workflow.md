...
- [ ] Cocos：Player 節點
  ...
  - [ ] 加 `PlayerAnimator.ts`（動畫切換 + 翻面，Phase 3 抽出。沒掛 → 不會切 clip、不會翻面，但物理仍正常）
    - [ ] Inspector：Animation 留空（自動找同節點 cc.Animation），或拉指定的 cc.Animation
    - [ ] Inspector：Flip Node 留空 → 翻 Player 節點；若想只翻 Sprite 子節點 → 拉那個子節點
    - [ ] Inspector：clipIdle / clipWalk / clipJump / clipFall 填動畫 clip 名稱（沒填 → 那狀態不切，不會錯）
- [ ] Play 測試：掉到地面 / 走 / 跳 / 雙跳
